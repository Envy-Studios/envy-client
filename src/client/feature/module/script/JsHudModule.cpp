...
JsHUDModule::JsHUDModule(std::string const& name, std::wstring const& displayName, std::wstring const& desc, int key,
                         bool resizable)
    : HUDModule(name, displayName, desc, SCRIPT, key, resizable) {
...
void JsHUDModule::preRender(bool mcRend, bool isPreview, bool isEditor) {
    ...
    Event ev { L"render",
               { isPreview ? Chakra::GetTrue() : Chakra::GetFalse(),
                 isEditor ? Chakra::GetTrue() : Chakra::GetFalse() } };
    dispatchEvent(ev);
