...
void Zoom::onRenderLevel(Event& evGeneric) {
    ...
    if (!shouldZoom) zoomModifier = 0.f;

    modifyTo = std::clamp(shouldZoom ? std::get<FloatValue>(modifier).value + zoomModifier : 1.f, 1.f, 60.f);

    // partial ticks
    float alpha = Envy::getRenderer().getDeltaTime();
    float lr = modifyTo;
    if (std::get<BoolValue>(hasAnim))
        lr = std::lerp(activeModifier, modifyTo, alpha * std::get<FloatValue>(animSpeed) / 10.f);
    activeModifier = lr;
