...
JsScreen::JsScreen(JsValueRef object, std::string name)
    : name(std::move(name)) {
...
void JsScreen::onClick(::Event& evG) {
    ...
    Event sEv { L"mouse",
                { Chakra::MakeInt(ev.getMouseButton()), Chakra::MakeInt(ev.getWheelDelta()),
                  ev.isDown() ? Chakra::GetTrue() : Chakra::GetFalse() } };
    auto ret = dispatchEvent(sEv);
    if (ret != JS_INVALID_REFERENCE) {
        ev.setCancelled(Chakra::GetBool(ret));
        Chakra::Release(ret);
