...
public:
    ...
    public:
        ...
        AsyncOperation(bool shouldRemove, JsValueRef callback, decltype(initFunc) initFunc, void* param = nullptr)
        ...
        virtual ~AsyncOperation() { JS::JsRelease(callback, nullptr); }
    };
    ...
    struct Resource {
        ...
        Resource(void* ptr, decltype(finalize) finalizer)
            : ptr(ptr, finalizer)
            , finalize(finalizer) {}
    };

    void addResource(void* object, decltype(Resource::finalize) finalizer) {
        this->resources.emplace_back(object, finalizer);
    };
