...
static std::array<FARPROC, 16> banList = {
    (FARPROC)VirtualProtect,   (FARPROC)VirtualProtectEx,   (FARPROC)mouse_event,      (FARPROC)keybd_event,
    (FARPROC)SendInput,        (FARPROC)CreateThread,       (FARPROC)OpenProcess,      (FARPROC)OpenProcessToken,
    (FARPROC)LoadLibraryA,     (FARPROC)LoadLibraryW,       (FARPROC)LoadLibraryExA,   (FARPROC)LoadLibraryExW,
    (FARPROC)GetModuleHandleA, (FARPROC)GetModuleHandleExA, (FARPROC)GetModuleHandleW, (FARPROC)GetModuleHandleExW,
};
...
public:
    ...
    JsNativeModule(class JsScript* owner)
    ...
    static JsValueRef CALLBACK getCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
        ...
        if (!JsScript::getThis()->getPlugin()->isTrusted() &&
            !JsScript::getThis()->getPlugin()->hasPermission(JsPlugin::UserPermission::SYSTEM_ACCESS)) {
    ...
    static JsValueRef CALLBACK callCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
        ...
        if (!Chakra::VerifyParameters({ { arguments[1], JsString }, { arguments[2], JsString } }))
            return JS_INVALID_REFERENCE;
        ...
        if (!JsScript::getThis()->getPlugin()->isTrusted())
            for (auto& banned : banList) {
                if (proc == banned) {
                    Chakra::ThrowError(L"This function is banned in non-trusted plugins.");
                    return JS_INVALID_REFERENCE;
                }
            }
        ...
        if (type == L"int64") {
            // int64_t
            auto res = ((any_func_int64_t)proc)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
                                                args[8], args[9], args[10], args[11], args[12], args[13], args[14]);
            return Chakra::MakeDouble(res);
        } else if (type == L"int32") {
            // int
            auto res = ((any_func_int32_t)proc)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
                                                args[8], args[9], args[10], args[11], args[12], args[13], args[14]);
            return Chakra::MakeInt(res);
        } else if (type == L"float32") {
            // float
            auto res =
                ((any_func_float32_t)proc)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
                                           args[8], args[9], args[10], args[11], args[12], args[13], args[14]);
            return Chakra::MakeDouble(res);
        } else if (type == L"float64") {
            // double
            auto res =
                ((any_func_float64_t)proc)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7],
                                           args[8], args[9], args[10], args[11], args[12], args[13], args[14]);
            return Chakra::MakeDouble(res);
        }
