...
JsErrorCode JsScript::runScript() {
    ...
    code = JS::JsRunScript(loadedScript.c_str(), util::fnv1a_32(util::WStrToStr(this->path.wstring())),
                           this->path.wstring().c_str(), nullptr);
    return code;
}
...
namespace {
    JsValueRef CALLBACK setTimeoutCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                           unsigned short argCount, void* callbackState) {
        if (!Chakra::VerifyArgCount(argCount, 3)) return Chakra::GetUndefined();
        auto sz = Chakra::VerifyParameters(
            { { arguments[1], JsValueType::JsFunction }, { arguments[2], JsValueType::JsNumber } });
        if (!sz) {
        ...
        auto tim = std::make_shared<JsScript::JsTimeout>(static_cast<int>(thi->timeouts.size() + 1),
                                                         static_cast<long long>(num), func);
    ...
    JsValueRef CALLBACK setIntervalCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
        if (!Chakra::VerifyArgCount(argCount, 3)) return Chakra::GetUndefined();
        ...
        JsScript* thi = reinterpret_cast<JsScript*>(callbackState);
        auto tim = std::make_shared<JsScript::JsTimeout>(static_cast<int>(thi->intervals.size() + 1),
                                                         static_cast<long long>(num), func);
        thi->timeouts.push_back(tim);
    ...
    JsValueRef CALLBACK clearTimeoutCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                             unsigned short argCount, void* callbackState) {
    ...
    JsValueRef CALLBACK clearIntervalCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
        if (!Chakra::VerifyArgCount(argCount, 2)) return Chakra::GetUndefined();
        if (!Chakra::VerifyParameters({ { arguments[1], JsValueType::JsNumber } })) return JS_INVALID_REFERENCE;
    ...
    JsValueRef CALLBACK getEnvCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                       unsigned short argCount, void* callbackState) {
        if (!Chakra::VerifyArgCount(argCount, 2)) return Chakra::GetUndefined();
    ...
    JsValueRef CALLBACK sleepCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                      unsigned short argCount, void* callbackState) {
        if (!Chakra::VerifyArgCount(argCount, 2)) return Chakra::GetUndefined();
    ...
    JsValueRef CALLBACK clientMessageCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
        JsValueRef undef;
    ...
    JsValueRef CALLBACK loadModule(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                   unsigned short argCount, void* callbackState) {
        ...
        if (!Chakra::VerifyParameters({ { arguments[1], JsValueType::JsString } })) {
            return undef;
        };
        ...
        if (!mod) {
            ...
            Logger::Warn("Terminating plugin {}! This is most likely caused by a circular dependency ({})->({})",
                         util::WStrToStr(thi->getPlugin()->getName()), thi->getRelativePath().string(),
                         util::WStrToStr(wPath));
    ...
    JsValueRef CALLBACK requestPermission(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                          unsigned short argCount, void* callbackState) {
        auto thi = reinterpret_cast<JsScript*>(callbackState);
        ...
        static std::wstring sysAccessMessage =
            util::StrToWStr("This script is requesting the System Access permission.\n\nPlease make sure you trust "
                            "this plugin!\nThis permission will give the plugin access to system functions, like "
                            "playing system sounds, ejecting the CD drive, and many other things.");
