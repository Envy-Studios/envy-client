...
public:
    ...
    void setDevice11(ID3D11Device* dev) { gameDevice11 = dev; }

    void setDevice12(ID3D12Device* dev) { gameDevice12 = dev; }
    ...
    void setFontFamily(std::wstring ws) { this->fontFamily = ws; }

    [[nodiscard]] std::wstring getFontFamily2() { return this->fontFamily2; }
    ...
    [[nodiscard]] float getDeltaTime() { return deltaTime; }
    ...
    [[nodiscard]] ID2D1DeviceContext* getDeviceContext() { return d2dCtx.Get(); }

    [[nodiscard]] ID2D1Device* getDevice() { return d2dDevice.Get(); }

    [[nodiscard]] ID2D1SolidColorBrush* getSolidBrush() { return solidBrush.Get(); }

    [[nodiscard]] ID2D1Factory3* getFactory() { return d2dFactory.Get(); }

    [[nodiscard]] IDWriteFactory* getDWriteFactory() { return dWriteFactory.Get(); }
    ...
    [[nodiscard]] ID2D1Bitmap1* copyCurrentBitmap() {
        ...
        HRESULT hr = d2dCtx->CreateBitmap(bitmapSize, nullptr, 0,
                                          D2D1::BitmapProperties1(D2D1_BITMAP_OPTIONS_TARGET, pixelFormat), &newBitmap);
        if (SUCCEEDED(hr)) {
            newBitmap->CopyFromBitmap(nullptr, myBitmap, nullptr);
        }
        return newBitmap;
    ...
    [[nodiscard]] ID2D1Effect*& getShadowEffect() { return *shadowEffect.GetAddressOf(); }

    [[nodiscard]] ID2D1Effect*& getAffineTransformEffect() { return *affineTransformEffect.GetAddressOf(); }

    [[nodiscard]] ID2D1Effect*& getBlurEffect() { return *blurEffect.GetAddressOf(); }
    ...
    [[nodiscard]] ID2D1Bitmap1* getBlurBitmap() { return getCopiedBitmap(); }

    [[nodiscard]] IWICImagingFactory2* getImagingFactory() { return this->wicFactory.Get(); }

    [[nodiscard]] ComPtr<IDWriteTextLayout> getLayout(IDWriteTextFormat* fmt, std::wstring const& str,
                                                      bool cache = false);
};
