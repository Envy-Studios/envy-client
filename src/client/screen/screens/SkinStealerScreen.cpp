...
uint64_t SkinStealerScreen::getPlayerListSignature() const {
    ...
    for (auto& item : *playerList) {
        ...
        for (uint64_t value : { static_cast<uint64_t>(image->width), static_cast<uint64_t>(image->height),
                                (image->bytes.size()), reinterpret_cast<uintptr_t>(image->bytes.data()) }) {
...
void SkinStealerScreen::pruneHeadCache() {
    ...
    for (std::unordered_map<std::string, HeadBitmap>::iterator it = headBitmaps.begin(); it != headBitmaps.end();) {
        if (activeSkins.contains(it->first))
            ++it;
        else
            it = headBitmaps.erase(it);
    }
}
...
SkinStealerScreen::HeadBitmap* SkinStealerScreen::getHeadBitmap(SDK::PlayerListEntry& entry) {
    ...
    if (std::unordered_map<std::string, HeadBitmap>::iterator found = headBitmaps.find(skinKey);
        found != headBitmaps.end() && found->second.bitmap) {
        return &found->second;
    }
    ...
    if (!dc ||
        FAILED(dc->CreateBitmap(
            ...
            bgra.data(), static_cast<UINT32>(playerHeadTextureSize) * 4u, &props, cached.bitmap.GetAddressOf()))) {
        return nullptr;
    }
...
std::vector<uint8_t> SkinStealerScreen::makePlayerHeadRgba(SDK::SkinImage const& image) const {
    ...
    std::vector<uint8_t> head(static_cast<size_t>(playerHeadTextureSize) * static_cast<size_t>(playerHeadTextureSize) *
                              4u);
    for (uint32_t y = 0; y < static_cast<uint32_t>(playerHeadTextureSize); ++y) {
        ...
        for (uint32_t x = 0; x < static_cast<uint32_t>(playerHeadTextureSize); ++x) {
            ...
            uint8_t* out =
                head.data() + ((static_cast<size_t>(y) * static_cast<size_t>(playerHeadTextureSize) + x) * 4u);
...
bool SkinStealerScreen::isValidSkinImage(SDK::SkinImage const* image) const {
    ...
    return pixelCount <= (std::numeric_limits<size_t>::max() / 4u) && image->bytes.size() >= pixelCount * 4u;
}
...
void SkinStealerScreen::onRender(Event&) {
    ...
    folderButtonRect = { closeButtonRect.left - 18.f * scale - folderButtonWidth, panelRect.top + 23.f * scale,
                         closeButtonRect.left - 18.f * scale, panelRect.top + 23.f * scale + folderButtonHeight };
    ...
    dc.drawText({ panelRect.left + pad, titleTop, folderButtonRect.left - 16.f * scale, titleTop + 31.f * scale },
                LocalizeString::get("client.module.skinStealer.name"), d2d::Colors::WHITE,
                Renderer::FontSelection::PrimaryLight, titleSize, DWRITE_TEXT_ALIGNMENT_LEADING,
                DWRITE_PARAGRAPH_ALIGNMENT_CENTER, false);
    dc.drawText(
        { panelRect.left + pad + 1.f * scale, titleTop + 30.f * scale, folderButtonRect.left - 16.f * scale,
          titleTop + 54.f * scale },
        util::FormatWString(LocalizeString::get(rows.size() == 1 ? "client.screen.skinStealer.playerCount.one"
                                                                 : "client.screen.skinStealer.playerCount.many"),
    ...
    dc.drawText(folderButtonRect, LocalizeString::get("client.screen.skinStealer.openFolder.name"), d2d::Colors::WHITE,
    ...
    if (rows.empty()) {
        dc.drawText(listRect, LocalizeString::get("client.screen.skinStealer.empty.name"),
                    d2d::Color::RGB(0xA8, 0xB4, 0xC8), Renderer::FontSelection::PrimaryRegular, 18.f * scale,
    ...
    for (size_t i = 0; i < rows.size(); i++) {
        ...
        d2d::Rect headRect { rowRect.left + 12.f * scale, rowRect.top + 10.f * scale,
                             rowRect.left + 12.f * scale + headSize, rowRect.top + 10.f * scale + headSize };
        ...
        row.getButtonRect = { rowRect.right - 14.f * scale - buttonWidth,
                              rowRect.top + (rowHeight - buttonHeight) * 0.5f, rowRect.right - 14.f * scale,
                              rowRect.top + (rowHeight + buttonHeight) * 0.5f };

        d2d::Rect nameRect { textLeft, rowRect.top + 14.f * scale, row.getButtonRect.left - 12.f * scale,
        ...
        dc.drawSingleLineFitted(nameRect, row.displayName, d2d::Colors::WHITE, Renderer::FontSelection::PrimaryRegular,
                                20.f * scale);
        if (!row.status.empty()) {
            dc.drawSingleLineFitted({ textLeft, rowRect.top + 41.f * scale, row.getButtonRect.left - 12.f * scale,
        ...
        if (showingFeedback) {
            buttonColor = row.statusSuccess ? d2d::Color::RGB(0x4C, 0xAF, 0x50) : d2d::Color::RGB(0xF4, 0x43, 0x36);
        } else if (buttonHovered) {
            buttonColor = accent;
        }

        dc.fillRoundedRectangle(row.getButtonRect, buttonColor, row.getButtonRect.getHeight() * 0.23f);
        dc.drawText(row.getButtonRect, LocalizeString::get("client.screen.skinStealer.get.name"), d2d::Colors::WHITE,
                    Renderer::FontSelection::PrimaryRegular, 16.f * scale, DWRITE_TEXT_ALIGNMENT_CENTER,
                    DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
    }
