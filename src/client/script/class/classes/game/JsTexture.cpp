...
JsTexture::JsTexture(std::wstring const& textureNameOrPath, bool gameTexture) {
    ...
    Envy::get().queueForDXRender([this, path](ID2D1DeviceContext* ctx) {
        ...
        conv->Initialize(pFrame.Get(), GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone, nullptr, 0.0,
...
std::filesystem::path JsTexture::tryGetRealPath(std::wstring const& oPath) {
    auto scriptPath = std::filesystem::path(JsScript::getThis()->getFolderPath());
