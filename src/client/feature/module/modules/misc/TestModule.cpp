...
TestModule::TestModule()
    : HUDModule("TestModule", L"TestModule", L"Module for testing new things.", GAMEHUD) {
...

    {
        ...
        if (col.numColors == 2) {
            ...
            const D2D1_GRADIENT_STOP stops[50] = {
                ...
                colors[color].get(),
                1.f,
                colors[(color + 1) % col.numColors].get(),
            };
            dc->CreateGradientStopCollection(stops, _countof(stops), gradientStopCollection.GetAddressOf());

        } else if (col.numColors == 3) {
            const D2D1_GRADIENT_STOP stops[] = { 0.f, d2d::Color(col.color1).get(), 0.5f, d2d::Color(col.color2).get(),
