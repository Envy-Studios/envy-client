...
ChunkBorders::ChunkBorders()
    : Module("ChunkBorders", LocalizeString::get("client.module.chunkBorders.name"),
...
namespace {
    void renderFace(MCDrawUtil3D& dc, float x1, float z1, float x2, float z2, float opacity) {
        ...
        for (float myY = mMinY + 2.f; myY <= mMaxY; myY += 2.f) {
            dc.drawLine({ x1, myY, z1 }, { x2, myY, z2 },
                        static_cast<int>(myY) % 16 == 0 ? Color(0.f, 0.f, 1.f, opacity)
...
void ChunkBorders::onRender3d(Event& evG) {
    ...
    MCDrawUtil3D dc { SDK::ClientInstance::get()->levelRenderer, ev.getScreenContext(),
                      std::get<BoolValue>(this->transparent) ? SDK::MaterialPtr::getSelectionOverlayMaterial()
    ...
    std::vector<std::pair<float, float>> outChunks = { { minX - 16.f, minZ - 16.f }, { minX, minZ - 16.f },
                                                       { minX + 16.f, minZ - 16.f }, { minX - 16.f, minZ },
                                                       { minX + 16.f, minZ },        { minX - 16.f, minZ + 16.f },
                                                       { minX, minZ + 16.f },        { minX + 16.f, minZ + 16.f } };
