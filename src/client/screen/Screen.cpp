...
void Screen::onRenderOverlay(Event& ev) {
    ...
    if (isActive() && this->tooltip.has_value()) {
        ...
        if (now - lastTooltipChange >= 500ms) {
            ...
            d2d::Rect textRect =
                dc.getTextRect(this->tooltip.value(), Renderer::FontSelection::PrimaryRegular, 15.f, 8.f);
            ...
            dc.drawText(textRect, this->tooltip.value(), d2d::Color(1.f, 1.f, 1.f, 0.8f),
                        Renderer::FontSelection::PrimaryRegular, 15.f, DWRITE_TEXT_ALIGNMENT_CENTER,
