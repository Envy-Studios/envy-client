...
ToggleSprintSneak::ToggleSprintSneak()
    : TextModule("ToggleSprintSneak", LocalizeString::get("client.textmodule.toggleSprintSneak.name"),
                 LocalizeString::get("client.textmodule.toggleSprintSneak.desc"), GAMEHUD, 400.f, 0, true) {
...
void ToggleSprintSneak::onTick(Event& evGeneric) {
    ...
    if (std::get<BoolValue>(sneak) && toggleSneaking) {
        ...
        left = L"Sprinting";
        /*if (input->front && !plr->getItemUseDuration() && !input->sneak && plr->getHunger() > 6.f) { // TODO: check
        CollidedHorizontally/vertically + hunger if (!input->sprintKey) {
...
void ToggleSprintSneak::onKey(Event& evGeneric) {
    ...
    if (ev.getKey() ==
        SDK::ClientInstance::get()->inputHandler->mappingFactory->defaultKeyboardLayout->findValue("sprint"))
