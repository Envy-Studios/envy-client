...
void Notifications::onRender(Event& evG) {
    ...
    D2DUtil dc;
    D2D1_SIZE_F ssize = Envy::getRenderer().getScreenSize();
    ...
    if (!toasts.empty()) {
        ...
        auto dur =
            static_cast<float>(std::chrono::duration_cast<std::chrono::milliseconds>(now - toast.createTime).count());
        ...
        d2d::Rect rec = { ss.centerX(textSize.x + xPad / 2), ss.top + yOffset, 0,
                          ss.top + yOffset + textSize.y + yPad / 2 };
        rec.right = rec.left + textSize.x + xPad / 2;

        dc.fillRoundedRectangle(rec, bgCol.asAlpha(opacity * bgCol.a), rec.getHeight() / 3.f);
        dc.drawRoundedRectangle(rec, outlineCol.asAlpha(opacity * outlineCol.a), rec.getHeight() / 3.f, 3,
                                DrawUtil::OutlinePosition::Outside);

        dc.drawText(rec, toast.message, textCol.asAlpha(opacity), font, fontSize, DWRITE_TEXT_ALIGNMENT_CENTER,
