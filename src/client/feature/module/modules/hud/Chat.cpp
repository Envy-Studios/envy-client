...
Chat::Chat()
    ...
    addSliderSetting("chatWidth", L"Chat Width", L"", chatWidth, FloatValue(100.f), FloatValue(1000.f),
                     FloatValue(10.f));
    ...
    addSliderSetting("animSpeed", L"Animation Time", L"The speed of the animation", animationSpeed, FloatValue(0.f),
                     FloatValue(5.f), FloatValue(0.1f));
    addSliderSetting("messageDuration", L"Message Duration", L"The duration of the message, excluding the animation",
                     messageDuration, FloatValue(1.f), FloatValue(15.f), FloatValue(0.5f));
...
void Chat::render(DrawUtil& ctx, bool isDefault, bool inEditor) {
    ...
    auto messageDuration =
        std::chrono::milliseconds(static_cast<int64_t>(std::get<FloatValue>(this->messageDuration).value * 1000));
    auto messageAnimDuration =
        std::chrono::milliseconds(static_cast<int64_t>(std::get<FloatValue>(this->animationSpeed).value * 1000));
    ...
    for (size_t i = 0; i < std::min(messages.size(), static_cast<size_t>(maxMessages)); i++) {
        ...
        if (tm - msg.timeCreated > messageDuration) {
            msg.animation = sineCurve(std::min(
                1.f, (messageAnimDuration.count() / (float)std::chrono::duration_cast<std::chrono::milliseconds>(
                                                        tm - msg.timeCreated - messageDuration)
                                                        .count()) -
                         1.f));
        } else if (tm - msg.timeCreated < messageAnimDuration && msg.duplicate < 2) {
            msg.animation =
                sineCurve((float)std::chrono::duration_cast<std::chrono::milliseconds>(tm - msg.timeCreated).count() /
                          std::chrono::duration_cast<std::chrono::milliseconds>(messageAnimDuration).count());
        } else {
            msg.animation = 1.f;
        }
        ...
        d2d::Rect tRect = { 0, y - tSize.y, windowWidth, y };

        ctx.fillRectangle(tRect, bgCol.asAlpha(msg.animation * bgCol.a));

        tRect = { (1.f - msg.animation) * -windowWidth, y - tSize.y, windowWidth * (msg.animation), y };

        y -= tSize.y;

        /*we will cache the message in d2d. it's more worth it*/
        ctx.drawText(tRect, text, fgCol.asAlpha(msg.animation * fgCol.a), Renderer::FontSelection::PrimaryRegular,
                     textSize, DWRITE_TEXT_ALIGNMENT_LEADING, DWRITE_PARAGRAPH_ALIGNMENT_NEAR, true);
    }
...
void Chat::onRenderLayer(Event& evG) {
    // TODO: This method is very scuffed. A better method is to actually disable the rendering of the chat or to make
    // the control invisible.
