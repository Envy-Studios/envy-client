...
JsErrorCode Chakra::CallFunction(JsValueRef func, JsValueRef* arguments, unsigned short argumentCount,
                                 JsValueRef* result) {
...
d2d::Rect Chakra::GetRectFromJs(JsValueRef obj) {
    return { static_cast<float>(Chakra::GetNumberProperty(obj, L"left")),
...
Vec2 Chakra::GetVec2FromJs(JsValueRef obj) {
    return { static_cast<float>(Chakra::GetNumberProperty(obj, L"x")),
...
Chakra::Result Chakra::VerifyParameters(std::initializer_list<ParamContainer> params, bool autoThrow) {
    ...
    for (auto const& param : params) {
        ...
        if (param.isOptional && (param.val == JS_INVALID_REFERENCE || trueType == JsUndefined || trueType == JsNull))
            continue;

        if (trueType != param.type) {
            std::wstring wstr = L"Parameter " + std::to_wstring(count) + L" must be of correct type (has " +
                                Chakra::GetTypeName(trueType) + L", needs " + Chakra::GetTypeName(param.type) + L")";
            if (autoThrow) {
                ThrowError(wstr);
            }
