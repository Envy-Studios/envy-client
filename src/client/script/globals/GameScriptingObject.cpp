...
#include "../JsScript.h"
#include "client/script/class/classes/JsVec2.h"
...
#include <client/script/class/classes/game/JsBlock.h>
...
void GameScriptingObject::initialize(JsContextRef ctx, JsValueRef parentObj) {
    ...
    Chakra::DefineFunc(object, getFOV, L"getFOV");
    Chakra::DefineFunc(object, getCameraPosition, L"getCameraPosition");
    Chakra::DefineFunc(object, captureCursor, L"captureCursor");
    Chakra::DefineFunc(object, releaseCursor, L"releaseCursor");
    Chakra::DefineFunc(object, isKeyDown, L"isKeyDown");
}
...
JsValueRef GameScriptingObject::worldExists(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef GameScriptingObject::worldGetName(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                             unsigned short argCount, void* callbackState) {
...
JsValueRef GameScriptingObject::worldGetPlayers(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    if (!SDK::ClientInstance::get()->getLocalPlayer()) {
        Chakra::ThrowError(L"World is not allowed to be used here");
        return Chakra::GetUndefined();
...
JsValueRef GameScriptingObject::worldGetEntList(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    if (!SDK::ClientInstance::get()->getLocalPlayer()) {
    ...
    for (auto& ent : entList) {
        ...
        if (ent->getRuntimeID() == 1) {
            JS::JsSetIndexedProperty(
                array, db,
                lplrc->construct(new JsEntity(ent->getRuntimeID(), JsEntity::AccessLevel::LocalPlayer), true));
...
JsValueRef GameScriptingObject::worldGetEntCount(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                 unsigned short argCount, void* callbackState) {
    if (!SDK::ClientInstance::get()->getLocalPlayer()) {
        Chakra::ThrowError(L"World is not available");
        return Chakra::GetUndefined();
...
JsValueRef GameScriptingObject::dimensionGetName(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                 unsigned short argCount, void* callbackState) {
    if (!SDK::ClientInstance::get()->getLocalPlayer()) {
...
JsValueRef GameScriptingObject::dimensionGetBlock(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                  unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 4)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsValueType::JsNumber },
                                    { arguments[2], JsValueType::JsNumber },
                                    { arguments[3], JsValueType::JsNumber } }))
        return JS_INVALID_REFERENCE;
...
JsValueRef GameScriptingObject::getMousePosCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                    unsigned short argCount, void* callbackState) {
    JsScript* script = JsScript::getThis();
...
JsValueRef GameScriptingObject::getScreenSizeCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                      unsigned short argCount, void* callbackState) {
    return JsScript::getThis()->getClass<JsVec2>()->construct(SDK::ClientInstance::get()->getGuiData()->screenSize);
}

JsValueRef GameScriptingObject::getInputBinding(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
...
JsValueRef GameScriptingObject::getWorldCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                 unsigned short argCount, void* callbackState) {
...
JsValueRef GameScriptingObject::getDimensionCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                     unsigned short argCount, void* callbackState) {
    // if (!SDK::ClientInstance::get()->minecraft->getLevel()) return Chakra::GetNull();
    JsContextRef ct;
    JS::JsGetCurrentContext(&ct);
...
JsValueRef GameScriptingObject::isInUICallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
    return SDK::ClientInstance::get()->minecraftGame->isCursorGrabbed() ? Chakra::GetFalse() : Chakra::GetTrue();
}

JsValueRef GameScriptingObject::getFOV(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                       unsigned short argCount, void* callbackState) {
    if (!SDK::ClientInstance::get()->getLocalPlayer()) {
        return Chakra::GetNull();
    }
...
JsValueRef GameScriptingObject::getCameraPosition(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                  unsigned short argCount, void* callbackState) {
    if (!SDK::ClientInstance::get()->getLocalPlayer()) {
        return Chakra::GetNull();
    }
...
JsValueRef GameScriptingObject::captureCursor(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
    SDK::ClientInstance::get()->grabCursor();
    return Chakra::GetNull();
}

JsValueRef GameScriptingObject::releaseCursor(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                              unsigned short argCount, void* callbackState) {
    SDK::ClientInstance::get()->releaseCursor();
    return Chakra::GetNull();
}

JsValueRef GameScriptingObject::isKeyDown(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                          unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 2)) return JS_INVALID_REFERENCE;
...
JsValueRef GameScriptingObject::sendChatCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
    ...
    Chakra::ThrowError(L"Permission denied to use sendChatMessage");
    return Chakra::GetUndefined();
}

JsValueRef GameScriptingObject::executeCommand(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 2)) return Chakra::GetUndefined();
    if (!Chakra::VerifyParameters({ { arguments[1], JsValueType::JsString } })) return JS_INVALID_REFERENCE;
    auto str = util::WStrToStr(Chakra::GetString(arguments[1]));

    if (str.size() > 250 || str.size() < 1 || str.at(0) != '/') return Chakra::GetUndefined();
...
JsValueRef GameScriptingObject::playSoundUI(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 4)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters(
            { { arguments[1], JsString }, { arguments[2], JsNumber }, { arguments[3], JsNumber } }))
        return JS_INVALID_REFERENCE;
...
JsValueRef GameScriptingObject::getLocalPlayerCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                       unsigned short argCount, void* callbackState) {
    auto lp = SDK::ClientInstance::get()->getLocalPlayer();
