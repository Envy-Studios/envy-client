...
struct Vec2 final {
    ...
    constexpr Vec2 operator-(Vec2 const& right) { return { x - right.x, y - right.y }; }
    ...
    [[nodiscard]] float magnitude() { return static_cast<float>(std::sqrt(x * x + y * y)); }
};
...
struct Vec3 final {
    ...
    constexpr Vec3 operator+(Vec3 const& right) const { return { right.x + x, right.y + y, right.z + z }; }

    constexpr Vec3 operator*(float right) const { return { x * right, y * right, z * right }; }

    [[nodiscard]] float magnitude() const { return static_cast<float>(std::sqrt(x * x + y * y + z * z)); }
...
struct Vec3i final {
    ...
    constexpr Vec3i(int x, int y, int z)
        ...
        , y(y)
        , z(z) {}
    constexpr Vec3i(Vec3 const& vec)
        : x(static_cast<int>(vec.x))
        , y(static_cast<int>(vec.y))
        , z(static_cast<int>(vec.z)) {}
};

// constexpr Vec3::Vec3(Vec3i const& vec) : x(static_cast<int>(std::round(vec.x))),
// y(static_cast<int>(std::round(vec.y))), z(static_cast<int>(std::round(vec.z))) {}
...
struct AABB final {
    ...
    constexpr AABB(Vec3 lower, Vec3 higher)
        : lower(lower)
        , higher(higher) {}

    Vec3 getCenter() { return (lower + higher) * 0.5f; }
...
struct Color {
    ...
    constexpr Color()
        ...
        , b(0.f)
        , a(1.f) {}
    Color(float pFloat[4])
        : r(pFloat[0])
        , g(pFloat[1])
        , b(pFloat[2])
        , a(pFloat[3]) {}

    static constexpr Color RGB(int r, int g, int b, int a = 255) {
