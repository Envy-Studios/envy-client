...
#include <client/script/class/classes/game/JsTextureClass.h>
...
JsValueRef D2DScriptingObject::useCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
...
JsValueRef D2DScriptingObject::fillRectCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 3, true, true)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsObject },
                                    { arguments[2], JsObject },
                                    { Chakra::TryGet(arguments, argCount, 3), JsNumber, true } }))
        return JS_INVALID_REFERENCE;
...
JsValueRef D2DScriptingObject::drawRectCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 4, true, true)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsObject },
                                    { arguments[2], JsObject },
                                    { arguments[3], JsNumber },
                                    { Chakra::TryGet(arguments, argCount, 4), JsNumber, true } }))
        return JS_INVALID_REFERENCE;
...
JsValueRef D2DScriptingObject::drawTextCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 5, true)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsObject },
                                    { arguments[2], JsString },
                                    { arguments[3], JsNumber },
                                    { arguments[4], JsObject } }))
        return JS_INVALID_REFERENCE;
    ...
    auto mLock = thi->lock();
    thi->operationsDirty.push_back(
        { thi->matrix, OpDrawText(rc, text, size, color, Renderer::FontSelection::PrimaryRegular,
                                  DWRITE_TEXT_ALIGNMENT_LEADING, DWRITE_PARAGRAPH_ALIGNMENT_NEAR) });
    return Chakra::GetUndefined();
}

JsValueRef D2DScriptingObject::drawTextFullCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                    unsigned short argCount, void* callbackState) {
    ...
    if (!Chakra::VerifyParameters({ { arguments[1], JsObject },
                                    ...
                                    { arguments[3], JsNumber },
                                    { arguments[4], JsObject },
                                    { arguments[5], JsNumber },
                                    { arguments[6], JsNumber } }))
        return JS_INVALID_REFERENCE;
    ...
    auto mLock = thi->lock();
    thisptr->operationsDirty.push_back(
        { thi->matrix, OpDrawText(rect, text, size, color, Renderer::FontSelection::PrimaryRegular,
                                  (DWRITE_TEXT_ALIGNMENT)align, (DWRITE_PARAGRAPH_ALIGNMENT)vertAlign) });
    return Chakra::GetUndefined();
}

JsValueRef D2DScriptingObject::drawImageCallback(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                 unsigned short argCount, void* callbackState) {
    if (!Chakra::VerifyArgCount(argCount, 6, true)) return JS_INVALID_REFERENCE;
    if (!Chakra::VerifyParameters({ { arguments[1], JsObject },
                                    { arguments[2], JsObject },
                                    { arguments[3], JsNumber },
                                    { arguments[4], JsNumber },
                                    { arguments[5], JsObject, true } }))
        return JS_INVALID_REFERENCE;
...
JsValueRef D2DScriptingObject::getTextSize(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                           unsigned short argCount, void* callbackState) {
...
JsValueRef D2DScriptingObject::setClippingRect(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                               unsigned short argCount, void* callbackState) {
...
JsValueRef D2DScriptingObject::restoreClippingRect(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                                   unsigned short argCount, void* callbackState) {
    auto thi = reinterpret_cast<D2DScriptingObject*>(callbackState);
...
JsValueRef D2DScriptingObject::drawTriangle(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    return Chakra::GetUndefined();
}

JsValueRef D2DScriptingObject::fillTriangle(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                            unsigned short argCount, void* callbackState) {
    return JsValueRef();
}

JsValueRef D2DScriptingObject::drawCircle(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                          unsigned short argCount, void* callbackState) {
    return JsValueRef();
}

JsValueRef D2DScriptingObject::fillCircle(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                          unsigned short argCount, void* callbackState) {
    return JsValueRef();
}

JsValueRef D2DScriptingObject::drawLine(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                        unsigned short argCount, void* callbackState) {
    return JsValueRef();
}

JsValueRef D2DScriptingObject::drawItem(JsValueRef callee, bool isConstructor, JsValueRef* arguments,
                                        unsigned short argCount, void* callbackState) {
