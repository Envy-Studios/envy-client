...
public:
    ...
    HUDModule(std::string const& name, std::wstring const& displayName, std::wstring const& description,
              Category category, int keybind = 0, bool resizable = true)
        : Module(name, displayName, description, category, keybind, true)
        , resizable(resizable) {
        ...
        if (resizable)
            addSliderSetting("scale", LocalizeString::get("client.hudmodule.props.size.name"), L"", scale,
    ...
    HUDModule(std::string const& name, LocalizedString const& displayName, LocalizedString const& description,
              Category category, int keybind = 0, bool resizable = true)
        : HUDModule(name, displayName.value(), description.value(), category, keybind, resizable) {
        setDisplayName(displayName);
        setDescription(description);
    }
    HUDModule(std::string const& name, LocalizedString const& displayName, std::wstring const& description,
              Category category, int keybind = 0, bool resizable = true)
        : HUDModule(name, displayName.value(), description, category, keybind, resizable) {
        setDisplayName(displayName);
    }
    HUDModule(std::string const& name, std::wstring const& displayName, LocalizedString const& description,
              Category category, int keybind = 0, bool resizable = true)
