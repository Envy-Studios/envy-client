...
bool Renderer::init(IDXGISwapChain* chain) {
    ...
    if (!swapChain4) {
        ...
        if (FAILED(swapChainHr) || !swapChain4) {
            Logger::Fatal("IDXGISwapChain4 QueryInterface failed: hr=0x{:08X}, chain=0x{:X}",
                          static_cast<unsigned>(swapChainHr), reinterpret_cast<uintptr_t>(chain));
            return false;
        }
    }
    ...
    if (forceDX11 || !gameDevice12) {
        ...
        if (queueDesc.Type != D3D12_COMMAND_LIST_TYPE_DIRECT || !sameComObject(gameDevice12.Get(), queueDevice.Get())) {
            Logger::Warn("Stored D3D12 command queue is not usable for D3D11On12: queue=0x{:X}, queueDevice=0x{:X}, "
                         "swapChainDevice=0x{:X}, type={} ({})",
                         reinterpret_cast<uintptr_t>(commandQueue.Get()),
                         reinterpret_cast<uintptr_t>(queueDevice.Get()),
                         reinterpret_cast<uintptr_t>(gameDevice12.Get()), commandQueueTypeName(queueDesc.Type),
        ...
        auto createD3D11On12Device = [&](UINT flags) {
            ...
            return D3D11On12CreateDevice(gameDevice12.Get(), flags, nullptr, 0, queues, _countof(queues), 0,
                                         d3dDevice.ReleaseAndGetAddressOf(), d3dCtx.ReleaseAndGetAddressOf(), nullptr);
        };
        ...
        if (FAILED(on12Hr) || !d3dDevice) {
            Logger::Fatal("D3D11On12CreateDevice failed: hr=0x{:08X} ({}), createFlags=0x{:X}, d3dDevice=0x{:X}, "
                          "d3dCtx=0x{:X}, gameDevice12=0x{:X}, commandQueue=0x{:X}, queueDevice=0x{:X}, queueType={}, "
                          "queueFlags=0x{:X}, queueNodeMask={}",
                          static_cast<unsigned>(on12Hr), dxgiErrorName(on12Hr), on12Flags,
                          reinterpret_cast<uintptr_t>(d3dDevice.Get()), reinterpret_cast<uintptr_t>(d3dCtx.Get()),
                          reinterpret_cast<uintptr_t>(gameDevice12.Get()),
                          reinterpret_cast<uintptr_t>(commandQueue.Get()),
                          reinterpret_cast<uintptr_t>(queueDevice.Get()), commandQueueTypeName(queueDesc.Type),
        ...
        if (FAILED(on12InterfaceHr) || !d3d11On12Device) {
            Logger::Fatal("ID3D11On12Device QueryInterface failed: hr=0x{:08X}, d3dDevice=0x{:X}, commandQueue=0x{:X}",
                          static_cast<unsigned>(on12InterfaceHr), reinterpret_cast<uintptr_t>(d3dDevice.Get()),
                          reinterpret_cast<uintptr_t>(commandQueue.Get()));
            lastFailedCommandQueue = commandQueue;
            commandQueue = nullptr;
            reqCommandQueue = true;
    ...
    if (FAILED(dxgiHr) || !dxgiDevice) {
        Logger::Fatal("IDXGIDevice QueryInterface failed: hr=0x{:08X}, d3dDevice=0x{:X}", static_cast<unsigned>(dxgiHr),
                      reinterpret_cast<uintptr_t>(d3dDevice.Get()));
        return false;
    }

    ThrowIfFailed(d2dFactory->CreateDevice(dxgiDevice.Get(), d2dDevice.GetAddressOf()));
    ThrowIfFailed(d2dDevice->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS,
                                                 d2dCtx.GetAddressOf()));
    ...
    for (int i = 0; i < bufferCount; i++) {
        ...
        if (gameDevice12.Get()) {
            ...
            ID3D11Resource* res;
            ThrowIfFailed(d3d11On12Device->CreateWrappedResource(backBuffer, &flags, D3D12_RESOURCE_STATE_RENDER_TARGET,
...
#pragma warning(pop)
    ...
    ThrowIfFailed(d2dCtx->CreateBitmap(bitmapSize, nullptr, 0,
...
void Renderer::releaseAllResources(bool flush, bool indep, bool dispatchCleanup) {
    ...
    if (isDX12) {
        ...
        if (d3dCtx) d3dCtx->OMSetRenderTargets(3, nullViews, nullptr);
    } else {
        ID3D11RenderTargetView* nullViews[] = { nullptr };
        if (d3dCtx) d3dCtx->OMSetRenderTargets(1, nullViews, nullptr);
    }
...
void Renderer::createTextFormats() {
    ...
    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily.c_str(), nullptr, DWRITE_FONT_WEIGHT_NORMAL,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->primaryFont.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily.c_str(), nullptr, DWRITE_FONT_WEIGHT_SEMI_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->primarySemilight.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily.c_str(), nullptr, DWRITE_FONT_WEIGHT_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->primaryLight.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily2.c_str(), nullptr, DWRITE_FONT_WEIGHT_NORMAL,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->secondaryFont.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily2.c_str(), nullptr, DWRITE_FONT_WEIGHT_SEMI_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->secondarySemilight.GetAddressOf()));

    ThrowIfFailed(dWriteFactory->CreateTextFormat(fontFamily2.c_str(), nullptr, DWRITE_FONT_WEIGHT_LIGHT,
                                                  DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, fontSize,
                                                  locale.c_str(), this->secondaryLight.GetAddressOf()));
...
void Renderer::createDeviceIndependentResources() {
    ...
    ThrowIfFailed(D2D1CreateFactory(D2D1_FACTORY_TYPE_MULTI_THREADED, __uuidof(ID2D1Factory3),
    ...
    ThrowIfFailed(CoCreateInstance(CLSID_WICImagingFactory, nullptr, CLSCTX_INPROC_SERVER,
                                   ...
                                   reinterpret_cast<void**>(this->wicFactory.GetAddressOf())));
}
...
ComPtr<IDWriteTextLayout> Renderer::getLayout(IDWriteTextFormat* fmt, std::wstring const& str, bool cache) {
    ...
    if (FAILED(dWriteFactory->CreateTextLayout(str.c_str(), static_cast<uint32_t>(str.size()), fmt, width, height,
                                               layout.GetAddressOf()))) {
        return {};
    }
